...
- `schema`: 入出力JSON Schemaの表示
- `testdata`: 人向けサンプル入力・ルールセットの生成
- `validate`: 入力の最低限検証
...
- `.local/marume-sample/case-ok.json`
- `.local/marume-sample/cases-basic.jsonl`
- `.local/marume-sample/rules-minimal.json`

その後、生成したサンプルをそのまま使って CLI を試せます。
...
`marume schema <name>` で JSON Schema を取得できます。`--help` に出る入力例・必須項目も同じ schema 定義から生成しています。
