...
func TestVersionは必須メタデータ欠落を入力エラーにする(t *testing.T) {
	tmpDir := t.TempDir()
	rulesPath := filepath.Join(tmpDir, "rules-missing-meta.sqlite")
	...
	if err := store.WriteSQLiteRuleSet(rulesPath, ruleSet); err != nil {
...
func TestClassifyBatchはルール読み込み失敗時に既存出力を壊さない(t *testing.T) {
	...
	err := cli.Run(
		context.Background(),
		[]string{"classify-batch", "--input", inputPath, "--output", outputPath, "--rules", filepath.Join(tmpDir, "missing-rules.sqlite")},
		strings.NewReader(""),
		&stdout,
		&stderr,
	)
