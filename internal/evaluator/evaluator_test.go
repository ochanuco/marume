...
func Testルールセット検証で必須メタデータと必須ルール項目を検出する(t *testing.T) {
	testCases := []struct {
		name    string
		ruleSet domain.RuleSet
	}{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := evaluator.ValidateRuleSet(tc.ruleSet)
			if !errors.Is(err, evaluator.ErrRuleDefinition) {
				t.Fatalf("ErrRuleDefinition を期待しましたが、実際は %v でした", err)
			}
		})
...
func Test単票系も必須メタデータ欠落をルール定義エラーとして返す(t *testing.T) {
	engine := evaluator.New(stubStore{
		ruleSet: domain.RuleSet{
			FiscalYear: 2026,
			Rules: []domain.Rule{
	...
	if _, err := engine.Classify(context.Background(), input); !errors.Is(err, evaluator.ErrRuleDefinition) {
		t.Fatalf("Classify は ErrRuleDefinition を期待しましたが、実際は %v でした", err)
	}
	if _, err := engine.Explain(context.Background(), input); !errors.Is(err, evaluator.ErrRuleDefinition) {
		t.Fatalf("Explain は ErrRuleDefinition を期待しましたが、実際は %v でした", err)
	}
}
